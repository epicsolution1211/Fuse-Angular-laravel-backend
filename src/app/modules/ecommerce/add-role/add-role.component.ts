...
export class AddRoleComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.roleId != null) {
            ...
            this.commonService.getRoleById(this.roleId).subscribe(res => {
                ...
                // this.roles.permissions.map(permission => {
                //     var finalPermissions= JSON.parse(permission.permissions);
                //     //finalPermissions = Object.keys(finalPermissions);
    ...
    onReadChange(id, isChecked) {
        ...
        if (isChecked) {
            ...
            if (id == 4) {
                ...
                let index = Tidx.permissions.indexOf("read");
                if (index !== -1) {
            ...
            if (id == 11) {
                const Tidx4 = this.permissions.find(x => x.id == 4);
                const index4 = Tidx4.permissions.indexOf("read");
                if (index4 !== -1) {
    ...
    onWriteChange(id, isChecked) {
        if (isChecked) {
            ...
            if (widx != undefined) {
                widx.permissions.push("write", "read");
            }
            // widx.permissions.push("write");
        } else {
            /*let idx = this.writePermissions.findIndex(x => x == id);
            this.writePermissions.splice(idx,1);  */
            const widx = this.permissions.find(x => x.id == id);
            if (widx.permissions.indexOf("write")) {
    ...
    onUpdateChange(id, isChecked) {
        if (isChecked) {
            ...
            widx.permissions.push("update", "read");
        } else {
            // let idx = this.updatePermissions.findIndex(x => x == id);
            // this.updatePermissions.splice(idx,1);
    ...
    onDeleteChange(id, isChecked) {
        if (isChecked) {
            ...
            // let idx = this.deletePermissions.findIndex(x => x == id);
            // this.deletePermissions.splice(idx,1);
            const uidx = this.permissions.find(x => x.id == id);
