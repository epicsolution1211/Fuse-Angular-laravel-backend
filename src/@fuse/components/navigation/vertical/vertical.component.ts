    ...
    constructor(
        ...
        if (this.role_id != undefined) {
            this.commonService.getRolePermissions(this.role_id).subscribe(res => {
                ...
                const dashboardIsThere = this.finalPermissions[0].includes("dashboard");
                if (!dashboardIsThere) {
                    const dashboard = 'dashboard';
                    this.finalPermissions[0].push(dashboard);
                }

                /** push static advisor data */
                // this.finalPermissions.push('advisor')
                // console.log("navigation", this.finalPermissions);
                    ...
                    if (defaultNavItem1?.children?.length != undefined) {
                        ...
                        defaultNavItem1.children.forEach((navItem) => {
                            if (this.role_id == 1 || this.role_id == 2) {
                                if (navItem.link == '/pool') {
                                    navItem.title = 'Feature requests';
                                }
                            }
                            const elementItem = this.finalPermissions.filter((element) => {
                                element.forEach(function (key, value) {

                                    if (element[value] == navItem.id) {
                        ...
                        if (dnavItem.children.length != 0) {
                            this.arr.push(dnavItem);
                        }
                    }
                });
            });
        }
        this.navigation = this.arr;
    }
    ...
    ngOnInit(): void {
        ...
        setTimeout(() => {
            ...
            if (mainNavigationComponent) {
                ...
                const menuItem = this._fuseNavigationService.getItem('mainNavigation.nav', mainNavigation);
                ...
                this.navigation.map((menu: any) => {
                    if (menu.id == "managment") {
                        menu.children.push(advisor);
                    }
                });
        ...
        // this.navigation = this._navigationMockApi['_defaultNavigation'];
        // console.log("static",this.navigation);
        // console.log("dynamic",this.arr);
        // this.navigation = this.arr;
